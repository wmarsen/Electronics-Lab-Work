...
def expected_surface_distance(theta): # Distance to surface from SERVO POV
    center_dist = 340   # mm when servo is centered
    edge_dist   = 460   # mm when servo is at maxAngle
    return center_dist + y_poltocart(340, theta)

def x_poltocart(theta):
    x = 340 * math.sin(theta)
    return x
...
def range_to_LED(x_mm, y_mm, distance_cm, LED_array, theta):
    ...
    led_index = int((x_mm + 340) / distancePerLED)
...
while True:
    ...
    theta = math.radians(servo_angle-135) # 0° = forward
    
    x = x_poltocart(theta)
    y = y_poltocart(dist_mm, theta)
    
    print("Servo angle: ", servo_angle - 135, "  range: mm ", dist_mm, "  x: mm ", x, "  y: mm ", y)
    
    LED_array = range_to_LED(x, y, dist_cm, LED_array, theta)
